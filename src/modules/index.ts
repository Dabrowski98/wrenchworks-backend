export { AuthModule } from './auth/auth.module';
export { AddressModule } from './address/address.module';
// export { AddressPersonModule } from './address-person/address-person.module';
// export { AddressWorkshopModule } from './address-workshop/address-workshop.module';
// export { CustomerModule } from './customer/customer.module';
// export { EmployeeModule } from './employee/employee.module';
// export { EmployeeTaskModule } from './employee-task/employee-task.module';
// export { JobModule } from './job/job.module';
// export { JobCategoryModule } from './job-category/job-category.module';
// export { PermissionSetModule } from './permission-set/permission-set.module';
// export { ReviewModule } from './review/review.module';
// export { ReviewResponseModule } from './review-response/review-response.module';
// export { ServiceModule } from './service/service.module';
// export { ServiceRequestModule } from './service-request/service-request.module';
// export { ServiceRequestJobModule } from './service-request-job/service-request-job.module';
// export { TaskModule } from './task/task.module';
// export { UserModule } from './user/user.module';
// export { UserReportModule } from './user-report/user-report.module';
// export { VehicleModule } from './vehicle/vehicle.module';
// export { VehicleBrandModule } from './vehicle-brand/vehicle-brand.module';
// export { VehicleDetailsModule } from './vehicle-details/vehicle-details.module';
// export { VehicleModelModule } from './vehicle-model/vehicle-model.module';
// export { WorkshopModule } from './workshop/workshop.module';
// export { WorkshopDetailsModule } from './workshop-details/workshop-details.module';
// export { WorkshopJobModule } from './workshop-job/workshop-job.module';
// export { WorkshopJobCategoryModule } from './workshop-job-category/workshop-job-category.module';


